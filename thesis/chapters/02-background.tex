...
The function expects an array of size \checkNum{five} bytes and returns the \checkNum{second} byte.
...
Furthermore, memory on the heap must be freed when it is not longer used, otherwise the program will eventually run out
...
It suggests jumping to very small so-called gadgets, which are sequences of two or few more machine instructions that
...
standard library packages instead of additional downloaded packages.
