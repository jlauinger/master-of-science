...
These bugs as well as suitable patches that were sent to the respective project maintainers are discussed in
...
To create a variation both of the amount of \unsafe{} usages and package size, the \checkNum{six} packages are selected
to include two packages each with few, medium and many \unsafe{} usages.
For every category, there is one small and one large package each.
Table~\ref{tbl:go-safer-evaluation-packages-stats} shows the packages used for this evaluation.

\input{assets/tables/chapter5/go-safer-evaluation-packages-stats.tex}

Each of the \checkNum{six} packages was then analyzed manually to find any insecure code that \toolSafer{} should
detect.
A \acrshort{CSV} file contained a positive or negative label for every line of code that contains a usage of the
\unsafe{} \acrshort{API}.
...
Similar to Section~\ref{subsec:go-safer:evaluation:labeled-usages}, the warnings and labels were then matches and
counted.
...
For \toolVet{}, this is is \textit{"possible misuse of unsafe.Pointer"} message generated by the \textit{unsafeptr}
...
dangerous.
The \toolGosec{} contain some false negatives because there are code lines in the evaluation data set that do
...
not \toolGosec{}.
Similarly, there are on true positives for \toolGosec{} in Table~\ref{tbl:go-safer-evaluation-packages} because there
...
Similarly, the inverse of that fraction (\checkNum{69\%})is seen as the accuracy of \toolGosec{} in that table.
It also has a rather high precision and recall but this is simply the result of generating warnings for every sample in
the set.
In the second evaluation on the \checkNum{six} packages, there is none that even achieves a defined and greater than
zero precision or recall for either \toolVet{} or \toolGosec{} because both tools do not report any true positives.
