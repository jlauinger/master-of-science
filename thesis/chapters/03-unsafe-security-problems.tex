...
\input{assets/figures/chapter3/protocol.tex}
\input{assets/figures/chapter3/payload.tex}
...
types.
Slices in Go are represented by a header structure that stores a pointer to the underlying data array, as well as
information about the current slice size and allocated size.
Strings are in fact just read-only byte slices.
...
Listing~\ref{unsafe-string-to-bytes-direct}, taken from the \textit{k8s.io/apiserver} package, shows a direct cast
from \textit{string} to \textit{[]byte}.
This is an incorrect cast that misses the \textit{Cap} field.
...
It has three not obvious but very dangerous problems that come with it.
...
Figure~\ref{fig:gc} shows a visualization of the \acrshort{GC} process.

\input{assets/figures/chapter3/gc.tex}

Now, when the \acrshort{GC} runs at the wrong time, the underlying data array used in the slice might have already been
collected, as illustrated by Figure~\ref{fig:gcrace-vuln}.
...
the previous section is that the Go compiler's escape analysis algorithm fails.
...
It is irrelevant whether the cast between the slice header which is constructed from scratch and the final slice is done
with a variable or in one statement, as shown in Listing~\ref{lst:string-to-bytes-1statement}.

\input{assets/listings/chapter3/string-to-bytes-1statement.tex}

In fact, the assembly code that gets produced by the compiler in these cases is exactly the same.
...
The third area of possible vulnerabilities contains types that have a different size or alignment on different
architectures.
