...
First, the dangers of conversions between types with architecture-dependent sizes are discussed.
...
The first area of possible vulnerabilities is about types that have a different size or alignment on different
architectures.
...
On top of incompatible field sizes, there could also be a different endianess on some architecture.
...
Go runtime that treats \textit{uintptr} references that are part of slices and strings as references.
...
If the bytes slice is modified, then arbitrary memory is changed.
...
In the following subsections, different ways of creating buffer overflows are discussed, and a concrete means of
...
A proof-of-concept implementation of a potential attacker showed that the method actually accesses invalid memory data
...
the syscall.
There are several alternative such gadgets in the Go standard library.
...
Because in contrast to the stack the code is located on an executable page, the CPU runs it and the code injection
attack succeeds.
