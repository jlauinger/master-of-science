...
\section{Quantitative Evaluation}\label{sec:go-geiger-evaluation}
...
Figure~\ref{fig:unsafe-import-depth} shows that most packages containing \unsafe{} usages are imported fairly early, however not directly at the
first level.
...
\todo{stricture comparisons better}
...
Figure~\ref{fig:label-examples} shows code examples and descriptions for each of the classes used to
...
\todo{line numbers are out of date. Replace with figure anyways.}
...
In the purpose dimension, the \textit{efficiency} (e.g. Line~2) class represents cases where unsafe is used only to
...
The \textit{avoid garbage collection} class (Line~48) contains usages to ask the Go compiler to lock a value while it is
used, for example, when calling a function written in assembly.
