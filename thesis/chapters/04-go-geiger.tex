...
It can be one of the four \unsafe{} package members \textit{Pointer}, \textit{Sizeof}, \textit{Offsetof}, and
...
Lines that are printed in green (the last \checkNum{two} lines in the table in Figure~\ref{fig:go-geiger-screenshot})
represent packages with no local \unsafe{} usages.
Red lines (e.g. the \checkNum{seventh} table row) are packages that directly contain \unsafe{} code, and white lines indicate
...
Furthermore, there is only one project with more than \checkNum{3,500} \unsafe{} usages, which is
...
They are a subset of the projects used for the empirical study (Table~\ref{tbl:projects}).
